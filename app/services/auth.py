...
def save_reset_code(user_id, code, expiry):
    ...
    conn.execute("UPDATE users SET reset_code = ?, reset_code_expiry = ? WHERE id = ?", (code, expiry, user_id))
    conn.commit()
    conn.close()
...
def verify_reset_code(email, code):
    conn = get_db_connection()
    user = conn.execute("SELECT * FROM users WHERE email = ? AND reset_code = ?", (email, code)).fetchone()
    conn.close()
...
def clear_reset_code(user_id):
    conn = get_db_connection()
    conn.execute("UPDATE users SET reset_code = NULL, reset_code_expiry = NULL WHERE id = ?", (user_id,))
    conn.commit()
    conn.close()
