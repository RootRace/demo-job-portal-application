...
def list_jobs():
    ...
    return render_template('index.html', jobs=jobs) if request.path == '/' else render_template('jobs.html', jobs=jobs, search=search, location=location)
...
def job_detail(job_id):
    ...
    has_applied = conn.execute('SELECT id FROM applications WHERE job_id = ? AND candidate_id = ?', (job_id, session['user_id'])).fetchone()
    conn.close()
...
def apply_job(job_id):
    ...
    existing_application = conn.execute('SELECT id FROM applications WHERE job_id = ? AND candidate_id = ?', (job_id, session['user_id'])).fetchone()
    if existing_application:
        flash('You have already applied for this job.', 'error')
        conn.close()
        return redirect(url_for('jobs.job_detail', job_id=job_id))

    conn.execute('INSERT INTO applications (job_id, candidate_id, status) VALUES (?, ?, ?)', (job_id, session['user_id'], 'applied'))
    conn.commit()
    conn.close()
